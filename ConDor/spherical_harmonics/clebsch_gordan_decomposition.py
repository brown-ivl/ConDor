...
def tf_clebsch_gordan_decomposition_(j1, j2, sparse=False, l_max=None, dtype=tf.float32):
    ...
    if sparse:
        ...
        coeffs = tf.convert_to_tensor(coeffs, dtype=dtype)
        return coeffs, idx
    else:
        return tf.convert_to_tensor(Q, dtype=dtype)
...
def decompose_(x, Q):
    ...
    s[-2] = -1
    s[-1] = c
    x = tf.reshape(x, shape=s)
    x = tf.einsum('ij,...jk->...ik', Q, x)
    y = []
    p = 0
...
def sparse_decompose_(x, coeffs, idx):
    ...
    x = tf.einsum('ij,...ijk->...ik', coeffs, x)
    y = []
    p = 0
    for J in range(abs(j1-j2), j1+j2+1):
        y.append(x[..., p:p+2*J+1, :])
        ...
        for j1 in range(self.l_max+1):
            for j2 in range(self.l_max+1):
                if self.l_max_out < abs(j1-j2):
                    ...
                    coeffs, idx = tf_clebsch_gordan_decomposition_(j1, j2, l_max=self.l_max_out,
                                                                   sparse=True, dtype=self.dtype)
                    self.Q[(j1, j2)] = [coeffs, idx]
                else:
                    self.Q[(j1, j2)] = tf_clebsch_gordan_decomposition_(j1, j2, l_max=self.l_max_out,
