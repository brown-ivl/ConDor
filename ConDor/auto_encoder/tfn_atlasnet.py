...
class TFN_multi(tf.keras.Model):
    def __init__(self, num_classes, num_frames = 1, num_capsules = 16):
        super(TFN_multi, self).__init__()
        ...
        for frame_num in range(num_frames):
            self.basis_mlp.append(set_mlp([64, 32], momentum=self.bn_momentum))
            self.basis_layer.append(Dense(units=self.basis_dim))

        self.code_dim = 64
        self.code_mlp = set_mlp([128, 32], momentum=self.bn_momentum)
        self.code_layer = Dense(units=self.code_dim)

        self.points_inv_mlp = set_mlp(units=[64, 32], momentum=self.bn_momentum)
    ...
    def call(self, x):
        ...
        # else:
        #     y = {'0': tf.ones((x.shape[0], 1024, 1, 1))}
        ...
        return capsules, points_inv, basis_list
...
if __name__=="__main__":
    ...
    model = TFN_multi(1024, 5)
    input_pcd = tf.random.normal((1, 1024, 3))
    capsules, points_inv, basis = model(input_pcd)
