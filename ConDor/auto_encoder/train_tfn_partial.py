...
def train_step_partial(x):
    ...
    # x_partial = tf_center(x_partial)
    pp_x = tf.gather_nd(x, x_partial_idx)
    pp_x = tf_center(pp_x)

    with tf.GradientTape() as tape:
        caps, inv, basis = autoencoder(x, training=False)
        caps = tf.stop_gradient(caps)
        inv = tf.stop_gradient(caps)
        basis = tf.stop_gradient(basis)


        p_caps, p_inv, p_basis = partial_autoencoder(x_partial, training=True)
        p_caps = tf.gather_nd(p_caps, kd_idx_inv)
        p_inv = tf_center(tf.gather_nd(p_inv, kd_idx_inv))

        pp_caps = tf.gather_nd(caps, x_partial_idx)
        pp_inv = tf_center(tf.gather_nd(inv, x_partial_idx))

        inv_partiality_loss = l2_loss_(p_inv, tf.stop_gradient(pp_inv))
        caps_partiality_loss = tf.keras.losses.CategoricalCrossentropy()(tf.stop_gradient(pp_caps), p_caps)


        s, u, v = tf.linalg.svd(p_basis)
        p_orth_basis = tf.matmul(u, v, transpose_b=True)
        p_orth_loss = tf.reduce_mean(tf.abs(p_basis - tf.stop_gradient(p_orth_basis)))
        p_y = tf.einsum('bvj,bmj->bvm', p_inv, p_orth_basis)
        p_y = tf.stack([p_y[..., 2], p_y[..., 0], p_y[..., 1]], axis=-1)
        p_l2_loss = l2_loss_(p_y, pp_x)
        ...
        # loss = chamfer_loss + 0.2*hausdorff_loss
        # loss = chamfer_loss
        loss = l2_loss + eq_loss + loc_loss + caps_chamf_loss + orth_loss + caps_partiality_loss + p_l2_loss + p_orth_loss + inv_partiality_loss
        # loss = orth_loss
        grad = tape.gradient(loss, autoencoder.trainable_variables)
        optimizer.apply_gradients(zip(grad, autoencoder.trainable_variables))
...
def train(trainset, valset, epochs, TEST):
    for epoch in range(epochs):
        ...
        for x in trainset:
            ...
            if TEST:
                l, l2, mrs, cl, hl, eq, loc, caps, o, z = test_step(x)
            else:
                l, l2, mrs, cl, hl, eq, loc, caps, o, z = train_step_partial(x)
        ...
        print('loss: ', loss_, ' l2_loss: ', l2_loss_, ' mrs_loss ', mrs_loss_, ' ch_l2: ', chamfer_loss_l2, ' h_l2: ', hausdorff_loss_l2,
              'eq: ', eq_loss, 'loc: ', loc_loss, 'caps: ', caps_chamf_loss, 'orth: ', orth_loss)
