...
WEIGHTS_PATH = "./results/tfn_capsules"
...
def train_step_partial(x):
    ...
    with tf.GradientTape() as tape:
        caps, inv, basis = autoencoder(x, training=True)
        p_caps, p_inv, p_basis = autoencoder(x_partial, training=True)
        p_caps = tf.gather_nd(p_caps, kd_idx_inv)
        p_inv = tf.gather_nd(p_inv, kd_idx_inv)

        pp_caps = tf.gather_nd(caps, x_partial_idx)
        pp_inv = tf.gather_nd(inv, x_partial_idx)

        inv_partiality_loss = l2_loss_(p_inv, pp_inv)
        caps_partiality_loss = tf.keras.losses.CategoricalCrossentropy()(pp_caps, p_caps)
        ...
        # loss = chamfer_loss + 0.2*hausdorff_loss
        # loss = chamfer_loss
        loss = l2_loss + eq_loss + loc_loss + caps_chamf_loss + orth_loss + inv_partiality_loss + caps_partiality_loss
        # loss = orth_loss
        grad = tape.gradient(loss, autoencoder.trainable_variables)
        optimizer.apply_gradients(zip(grad, autoencoder.trainable_variables))
    ...
    for epoch in range(epochs):
        ...
        for x in trainset:
            ...
            if TEST:
                ...
                l, l2, mrs, cl, hl, eq, loc, caps, o, z = train_step(x)
