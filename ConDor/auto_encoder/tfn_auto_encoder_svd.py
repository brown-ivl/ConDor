...
class TFN(tf.keras.Model):
    ...
    def call(self, x):
        ...
        """
        c_inv = tf.reduce_mean(points_inv, axis=1, keepdims=True)
        c_x = tf.reduce_mean(x, axis=1, keepdims=True)
        x_c = tf.subtract(x, c_x)
        inv_c = tf.subtract(points_inv, c_inv)
        xyt = tf.einsum('bvi,bvj->bij', inv_c, x_c)
        s, u, v = tf.linalg.svd(xyt)
        R = tf.matmul(v, u, transpose_b=True)
        d = tf.linalg.det(R)
        o = tf.ones([R.shape[0], 2])
        ...
        v = tf.multiply(v, d_)
        R = tf.matmul(v, u, transpose_b=True)
        y = tf.einsum("bij,bvj->bvi", R, inv_c)
        y = tf.add(y, c_inv)
        """
        sigma_x = sigma(x)
        x = tf.divide(x, sigma_x)
        inv = var_normalize(points_inv)
        R = registration(inv, x)
        y = tf.einsum('bij,bvj->bvi', R, inv)
        y = tf.multiply(y, sigma_x)
        inv = tf.multiply(inv, sigma_x)
        return y, inv, R
