...
class TFN(tf.keras.Model):
    def __init__(self, num_classes):
        ...
        self.invar_embedding_mlp = set_mlp(units=[32], momentum=self.bn_momentum)
        self.points_inv_layer = Dense(units=self.basis_dim)

        self.invar_template_mlp = set_mlp(units=[128, 64], momentum=self.bn_momentum)
        self.invar_template_layer = Dense(units=self.basis_dim)


        self.invar_template_decode_mlp = set_mlp(units=[128, 64], momentum=self.bn_momentum)
        self.invar_template_decode_layer = Dense(units=self.basis_dim)

        # self.reduce_invar_code_layer = Dense(units=32)

        self.invar_template_expand_layer = Dense(units=16)
    ...
    def call(self, inputs):
        ...
        tfn = {'0': tf.ones((X.shape[0], X.shape[1], 1, 1))}

        for i in range(len(self.radius)):
            tfn["source points"] = points[i]
            tfn["target points"] = points[i + 1]
            tfn["patches idx"] = grouped_points[i]["patches idx source"]
            tfn["patches dist source"] = grouped_points[i]["patches dist source"]
            tfn["kernels"] = kernels[i]
            # w = gauss_normalization(y["patches dist source"], 1./3.)



            if '1' in tfn:
                tfn['1'] = tf.concat([tfn['1'], yzx[i]], axis=-1)
            else:
                tfn['1'] = yzx[i]

            tfn = self.conv_layers[i](tfn)



            if '1' in tfn:
                tfn['1'] = tf.concat([tfn['1'], yzx[i + 1]], axis=-1)
            else:
                tfn['1'] = yzx[i + 1]

            tfn = apply_layers(tfn, self.equivariant_weights[i])
            tfn = SphericalHarmonicsEval(l_max=self.l_max_out[i], base=self.S2).compute(tfn)
            tfn = self.bn[i](tfn)
            tfn = Activation('relu')(tfn)
            tfn = apply_mlp(tfn, self.mlp[i])
            if i < len(self.radius) - 1:
                tfn = SphericalHarmonicsCoeffs(l_max=self.l_max_out[i], base=self.S2).compute(tfn)

        tfn = tf.reduce_max(tfn, axis=1, keepdims=False)

        tfn_ = tf.stop_gradient(tfn)
        basis = apply_mlp(tfn_, self.basis_mlp)
        basis = self.basis_layer(basis)
        basis = type_1(basis, self.S2)
        basis = tf.linalg.l2_normalize(basis, epsilon=1e-4)

        latent_code = apply_mlp(tfn, self.code_mlp)
        ...
        zernike_monoms_ = zernike_monoms(points[0], self.l_max_out[-1])

        points_code = []
        for l in latent_code:
            p = tf.einsum('bmi,bvmj->bvij', latent_code[l], zernike_monoms_[int(l)])
            shape = list(p.shape)
            shape = shape[:-1]
            shape[-1] = -1
        ...
        invar_embedding = apply_mlp(points_code, self.invar_embedding_mlp)
        invar_embedding = self.points_inv_layer(invar_embedding)
        ied = tf.einsum('bvj,bmj->bvm', invar_embedding, basis)
        invar_embedding_decode = tf.stack([ied[..., 2], ied[..., 0], ied[..., 1]], axis=-1)

        invar_template = apply_mlp(points_code, self.invar_template_mlp)
        invar_template = self.invar_template_layer(invar_template)
        it = self.invar_template_expand_layer(invar_template)

        invar_latent_code = norms(latent_code)

        invar_latent_code = tf.expand_dims(invar_latent_code, axis=1)
        invar_latent_code = tf.tile(invar_latent_code, (1, invar_template.shape[1], 1))

        invar_template_decode = tf.concat([it, invar_latent_code], axis=-1)
        invar_template_decode = apply_mlp(invar_template_decode, self.invar_template_decode_mlp)
        invar_template_decode = self.invar_template_decode_layer(invar_template_decode)

        itd = tf.einsum('bvj,bmj->bvm', invar_template_decode, basis)
        invar_template_decode = tf.stack([itd[..., 2], itd[..., 0], itd[..., 1]], axis=-1)



        invar_embedding = tf.split(invar_embedding, axis=0, num_or_size_splits=2)
        ie_x = invar_embedding[0]
        ie_y = invar_embedding[1]

        invar_embedding_decode = tf.split(invar_embedding_decode, axis=0, num_or_size_splits=2)
        ied_x = invar_embedding_decode[0]
        ied_y = invar_embedding_decode[1]

        invar_template = tf.split(invar_template, axis=0, num_or_size_splits=2)
        it_x = invar_template[0]
        it_y = invar_template[1]

        invar_template_decode = tf.split(invar_template_decode, axis=0, num_or_size_splits=2)
        itd_x = invar_template_decode[0]
        itd_y = invar_template_decode[1]

        return ie_x, ie_y, ied_x, ied_y, it_x, it_y, itd_x, itd_y
