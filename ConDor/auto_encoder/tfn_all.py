...
class TFN_all(tf.keras.Model):
    def __init__(self, num_classes, num_frames = 1, num_capsules = 16):
        ...
        self.scale_mlp = set_mlp(units = [64], momentum=self.bn_momentum)
    ...
    def call(self, x):
        ...
        scale = 10 * tf.nn.sigmoid(type_0(scale, self.S2))
        print(scale.shape, "Scale prediction shape")
        ...
        out = {"scale": scale, "translation": translation_list, "caps": capsules, "points_inv": points_inv, "basis_list": basis_list}
        return out
