...
def scale_equivariant_features(x):
    n = []
    for l in x:
        print(l)
        if type(l) == int and l != 0:
            l_ = float(int(l))
            nl = tf.pow(tf.reduce_sum(x[l]*x[l], axis=-2, keepdims=False), 1. / (2.*l_))
            n.append(nl)
    return tf.concat(n, axis=-1)
...
class TFN_all(tf.keras.Model):
    def __init__(self, num_classes, num_frames = 1, num_capsules = 16):
        ...
        self.scale_mlp = set_mlp(units = [16], momentum = self.bn_momentum)
        self.scale_layer = Dense(units = 1)
    ...
    def call(self, x):
        ...
        z_deg = scale_equivariant_features(z)
        ...
        y_scale = tf.stop_gradient(z_deg)
        scale = (apply_mlp(y_scale, self.scale_mlp))
        scale = tf.reduce_mean(scale, axis = -2, keepdims = True)
        scale = self.scale_layer(scale)
        scale = tf.nn.sigmoid(scale) * 3
        ...
        out = {"translation": translation_list, "caps": capsules, "points_inv": points_inv, "basis_list": basis_list, "scale": scale}
        return out
...
if __name__=="__main__":
    ...
    input_pcd = tf.random.normal((2, 1024, 3))
    out_dict = model(input_pcd)
    
    for key in out_dict.keys():
        if key != "basis_list" and key != "translation":
