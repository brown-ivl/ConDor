...
def patch_max_pooling(x, patches_idx, r, r_threshold=None):
    X = tf.gather_nd(x, patches_idx)
    if r_threshold is not None:
        X_min = tf.reduce_min(X, axis=2, keepdims=True)
        c = tf.less_equal(r, r_threshold)
        c = tf.reshape(c, list(c.shape) + [1, 1])
        X = tf.where(c, X, X_min)
    return tf.reduce_max(X, axis=2, keepdims=False)



def patch_avg_pooling(x, patches_idx, r, r_threshold=None):
    X = tf.gather_nd(x, patches_idx)
    if r_threshold is not None:
        c = tf.less_equal(r, r_threshold)
        c = tf.reshape(c, list(c.shape) + [1, 1])
        w = tf.reduce_sum(c, axis=2, keepdims=False)
        X = tf.where(c, X, 0.0)
        X = tf.reduce_sum(X, axis=2, keepdims=False)
        y = tf.divide(X, w)
        return y

    return tf.reduce_mean(X, axis=2, keepdims=False)

class SPH_VNN_POINTNET(tf.keras.Model):
    def __init__(self, num_classes):
        super(SPH_VNN_POINTNET, self).__init__()
        ...
        self.num_points = [1024, 512, 128, 16]
    ...
    def call(self, x):
        ...
        y = SphericalHarmonicsEval(l_list=[1], base=self.S2).compute(y)
        for i in range(len(self.radius)):
            ...
            y = patch_max_pooling(y, patches_idx, patches_dist_source, None)
            ...
            # y = kd_pooling_2d(y, pool_size=int(self.num_points[i]/self.num_points[i]), pool_mode='max')
            y = SphericalHarmonicsCoeffs(l_max=self.l_max_out[i], base=self.S2).compute(y)

            """
            for l in y:
                if l in y_prev:
                    y[l] = tf.concat([y[l], y_prev[l]], axis=-1)
            """
