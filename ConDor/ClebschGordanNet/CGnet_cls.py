...
def norms(x):
    ...
    return n
"""
def clebsh_gordan_product(x, y, cg_decomposer):
    types_x = []
    split_size_x = []
    types_y = []
    split_size_y = []
    ...
    for l in x:
        if l.isnumeric():
            types_x.append(int(l))
            split_size_x.append(2*int(l)+1)
            X.append(x[l])
    for l in y:
        if l.isnumeric():
            types_y.append(int(l))
            split_size_y.append(2 * int(l) + 1)
            Y.append(y[l])

    X = tf.expand_dims(tf.concat(X, axis=-2), axis=-3)
    Y = tf.expand_dims(tf.concat(Y, axis=-2), axis=-2)
    XY = tf.multiply(X, Y)

    Z = dict()
    XY = tf.split(XY, num_or_size_splits=split_size_x, axis=-3)
    for u in XY:
        u = tf.split(u, num_or_size_splits=split_size_y, axis=-2)
        for v in u:
            w = cg_decomposer.decompose(v)
            for l in w:
                if l in Z:
                    Z[l].append(w[l])
                else:
                    Z[l] = []
    for l in Z:
        Z[l] = tf.concat(Z[l], axis=-1)
    return Z
"""

def clebsh_gordan_product(x, cg_decomposer):
    Z = dict()
    ...
    for j in x:
        if j.isnumeric():
            for k in x:
                if k.isnumeric():
                    pairs.append((min(int(j), int(k)), max(int(j), int(k))))

    pairs = set(pairs)

    for p in pairs:
        xij = tf.multiply(tf.expand_dims(x[str(p[0])], axis=-3), tf.expand_dims(x[str(p[1])], axis=-2))
        y = cg_decomposer.decompose(xij)
        for l in y:
            if l in Z:
                Z[l].append(y[l])
            else:
...
def set_equivariant_weights(units, types):
    w = dict()

    for l in types:
        if l == '0':
            w['0'] = Dense(units=units)
        else:
            w[str(l)] = Dense(units=units, use_bias=False)
    return w


def apply_equvariant_weights(x, w):
    y = dict()
    for l in w:
        if l.isnumeric():
            y[l] = w[l](x[l])
...
class TFN(tf.keras.Model):
    def __init__(self, num_classes):
        ...
        for i in range(len(self.l_max_out)):
            self.cg.append(tf_clebsch_gordan_decomposition(l_max=self.l_max_out[i],
                                                           l_max_out=self.l_max_out[i]))
        ...
        for i in range(len(self.radius)):
            ...
            self.equivariant_weights.append(set_equivariant_weights(self.equivariant_units[i], types=types))

            """
            self.equivariant_weights.append(set_norm_activation_layers_sphere(units=self.equivariant_units[i],
    ...
    def call(self, x):
        ...
        for i in range(len(self.radius)):
            ...
            y = apply_equvariant_weights(y, self.equivariant_weights[i])
            y = clebsh_gordan_product(y, self.cg[i])
