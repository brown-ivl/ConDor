...
class SegmentationProvider(tf.keras.utils.Sequence):
    ...
    def __init__(self, files_list, data_path, n_points, samples_ratio=1.,
                 batch_size=32, preprocess=list(), shuffle=True):
        'Initialization'
        ...
        self.part_labels = np.zeros((self.data.shape[0], self.data.shape[1]))
        self.part_labels[self.data[:, :, 2] <= 0] = 0
        self.part_labels[self.data[:, :, 2] > 0] = 1
        self.part_labels = np.expand_dims(self.part_labels, axis=-1)
        self.class_labels = np.zeros((self.data.shape[0]))
    ...
    def __data_generation(self, indexes):
        ...
        class_labels = np.zeros((self.batch_size,))
        class_labels = tf.keras.utils.to_categorical(class_labels, num_classes=1)
        X = self.data[indexes, ...]
        X = X[:, idx, ...]
        y = np.zeros((self.batch_size, X.shape[1]), dtype=np.int32)
        y[X[:, :, 2] <= 0] = 0
        y[X[:, :, 2] > 0] = 1
