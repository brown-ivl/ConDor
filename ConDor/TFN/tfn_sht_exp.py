...
def equivariant_exp(x, u, v, weights=None):

    y = dict()
    for l in u:
        if l.isnumeric():
            uvl = tf.einsum('bik,bjk->bijk', u[l], v[l])
            vul = tf.transpose(uvl, (0, 2, 1, 3))
            el = (uvl - vul)/2.
            if weights is not None:
                el = weights[l](el)
            el = el[..., 0]
            el = tf.linalg.expm(el)
            el = tf.stop_gradient(el)
            y[l] = tf.matmul(el, x[l])
    return y
...
class TFN(tf.keras.Model):
    def __init__(self, num_classes):
        ...
        self.exp_weights2 = set_sphere_weights(1, types=types)
    ...
    def call(self, x):
        ...
        y1 = apply_layers(y, self.exp_weights1)
        y2 = apply_layers(y, self.exp_weights2)



        y = equivariant_exp(y, y1, y2, weights=None)
        # y = stack_eq(y)
