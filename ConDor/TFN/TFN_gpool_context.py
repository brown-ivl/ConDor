...
class TFN(tf.keras.Model):
    def __init__(self, num_classes):
        ...
        self.g_mlp_units = [[8, 8], [16, 16], [32, 32]]
        ...
        for i in range(len(self.radius)):
            ...
            if i < len(self.radius) - 1:
                self.gmlp.append(set_mlp(self.g_mlp_units[i], self.bn_momentum))
    ...
    def call(self, x):
        ...
        for i in range(len(self.radius)):
            ...
            y = self.bn[i](y)
            y_ = Activation('relu')(y)
            y = apply_mlp(y_, self.mlp[i])
            if i < len(self.radius) - 1:
                yg = apply_mlp(y_, self.gmlp[i])
                yg = tf.reduce_max(yg, axis=1, keepdims=True)
                yg = tf.tile(yg, (1, y.shape[1], 1, 1))
                y = tf.concat([y, yg], axis=-1)
