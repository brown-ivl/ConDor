...
from pooling import kd_pooling_2d, kd_pooling_1d, kdtree_indexing, aligned_kdtree_indexing, kdtree_indexing_, aligned_kdtree_indexing_
...
class TFN(tf.keras.Model):
    ...
    def call(self, x):
        ...
        # x = kdtree_indexing_(x)
        x, idx, inv_idx, v = aligned_kdtree_indexing_(X)
        N = tf.einsum('bij,bvi->bvj', v, N)
        N = tf.gather_nd(N, idx)
        Nyzx = tf.stack([N[..., 1], N[..., 2], N[..., 0]], axis=-1)
        ...
        y = {'0': tf.ones((x.shape[0], x.shape[1], 1, 1)), '1': tf.expand_dims(Nyzx, axis=-1)}
