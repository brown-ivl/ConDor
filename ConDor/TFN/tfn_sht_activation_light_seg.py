...
class SegNet(tf.keras.Model):
    def __init__(self, num_classes):
        ...
        # self.radius = [0.15, 0.30, 0.6]

        self.bounded = [True, True, True]
        self.num_points = [1024, 512, 128, 32]

        self.patch_size = [64, 64, 64]
        self.spacing = [0, 0, 0]

        # self.mlp_units = [[64, 64, 128], [128, 128, 256], [256, 512, 1024]]
        ...
        # self.mlp_units = [[48, 48], [96, 96], [256, 512, 1024]]
        ...
        for i in range(len(self.radius)):
            ...
            self.eval.append(DodecahedronEval(l_max=self.l_max_out, dodecahedron=self.dodecahedron))
            self.coeffs.append(DodecahedronCoeffs(l_max=self.l_max_coeffs[i], dodecahedron=self.dodecahedron))
        ...
        for i in range(len(self.radius)):
            ...
            types = [str(l) for l in range(self.l_max_out + 1)]
            self.equivariant_weights_up.append(set_sphere_weights(self.equivariant_units_up[i], types=types))
        ...
        self.S2 = tf_fibonnacci_sphere_sampling(32)
    def call(self, x):
        # pts_input, _, kd_idx_inv, _ = aligned_kdtree_indexing_(x[0])
        ...
        for i in range(len(self.radius)):
            ...
            # y = self.eval[i](y)
            # y = DodecahedronEval(l_max=self.l_max_out, dodecahedron=self.dodecahedron)(y)
            y = SphericalHarmonicsEval(l_max=self.l_max_out, base=self.S2).compute(y)
            y = self.bn[i](y)
            y = Activation('relu')(y)
            # y = apply_mlp(y, self.mlp[i], axes=None, mode='or')
            # if i < len(self.radius) - 1:

            # y = self.coeffs[i](y)
            # y = DodecahedronCoeffs(l_max=self.l_max_coeffs[i], dodecahedron=self.dodecahedron)(y)
            y = SphericalHarmonicsCoeffs(l_max=self.l_max_coeffs[i], base=self.S2).compute(y)
        ...
        for i in range(len(self.radius)-1):
            ...
            y = apply_layers(y, self.equivariant_weights_up[i])
            # y = DodecahedronEval(l_max=self.l_max_out, dodecahedron=self.dodecahedron)(y)
            y = SphericalHarmonicsEval(l_max=self.l_max_out, base=self.S2).compute(y)
            y = self.bn_up[i](y)
            y = Activation('relu')(y)
            # y = apply_mlp(y, self.mlp_up[i], axes=None, mode='or')
            # y = DodecahedronCoeffs(l_max=self.l_max_out, dodecahedron=self.dodecahedron)(y)
            y = SphericalHarmonicsCoeffs(l_max=self.l_max_out, base=self.S2).compute(y)
        ...
        if '1' in y:
            ...
            y['1'] = yzx[0]
        y = apply_layers(y, self.equivariant_weights_up[-1])
        # y = DodecahedronEval(l_max=self.l_max_out, dodecahedron=self.dodecahedron)(y)
        y = SphericalHarmonicsEval(l_max=self.l_max_out, base=self.S2).compute(y)
        y = self.bn_up[-1](y)
        y = Activation('relu')(y)
